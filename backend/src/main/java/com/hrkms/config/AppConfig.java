...
public class AppConfig {
    ...
    @Bean
    CommandLineRunner initData(KnowledgeItemRepository itemRepo, UserRepository userRepo, TagRepository tagRepo, BCryptPasswordEncoder encoder) {
        return args -> {
            // ========================
            // SEED DEFAULT USERS
            // ========================
            ...
            // ========================
            if (tagRepo.count() == 0) {
                List<Tag> seedTags = List.of(
                    Tag.builder().name("leave").description("Nghỉ phép, nghỉ lễ").build(),
                    Tag.builder().name("benefit").description("Phúc lợi, bảo hiểm").build(),
                    Tag.builder().name("contract").description("Hợp đồng lao động").build(),
                    Tag.builder().name("payroll").description("Lương, thưởng, phụ cấp").build(),
                    Tag.builder().name("tax").description("Thuế TNCN, giảm trừ gia cảnh").build(),
                    Tag.builder().name("onboarding").description("Quy trình nhận việc nhân viên mới").build(),
                    Tag.builder().name("working_time").description("Giờ làm việc, WFH").build(),
                    Tag.builder().name("behavior").description("Quy tắc ứng xử, kỷ luật").build()
                );
                tagRepo.saveAll(seedTags);
                System.out.println("✅ Initialized " + seedTags.size() + " tags");
            }
            ...
            // Build a lookup map: name -> Tag entity
            Map<String, Tag> tagMap = tagRepo.findAll().stream()
                    .collect(Collectors.toMap(Tag::getName, Function.identity()));

            // Helper to resolve tag names to Tag entities
            java.util.function.BiFunction<Map<String, Tag>, String[], List<Tag>> tags =
                    (map, names) -> java.util.Arrays.stream(names).map(map::get).collect(Collectors.toList());
            ...
            var pol1 = KnowledgeItem.builder()
                ...
                .tags(tags.apply(tagMap, new String[]{"leave", "benefit", "working_time"})).audience("All employees")
            ...
            items.add(KnowledgeItem.builder()
                ...
                .tags(tags.apply(tagMap, new String[]{"working_time", "benefit"})).audience("All employees")
            ...
            items.add(KnowledgeItem.builder()
                ...
                .tags(tags.apply(tagMap, new String[]{"behavior", "contract"})).audience("All employees, Line Managers")
            ...
            items.add(KnowledgeItem.builder()
                ...
                .tags(tags.apply(tagMap, new String[]{"benefit", "payroll", "tax"})).audience("All employees")
            ...
            items.add(KnowledgeItem.builder()
                ...
                .tags(tags.apply(tagMap, new String[]{"behavior", "contract"})).audience("All employees")
            ...
            items.add(KnowledgeItem.builder()
                ...
                .tags(tags.apply(tagMap, new String[]{"payroll", "tax", "contract"})).audience("All employees")
            ...
            items.add(KnowledgeItem.builder().id("FAQ-001").title("Tôi còn bao nhiêu ngày phép?").type("FAQ")
                .tags(tags.apply(tagMap, new String[]{"leave", "working_time"})).audience("All employees")
            ...
            items.add(KnowledgeItem.builder().id("FAQ-002").title("Ngày nào nhận lương? Payslip ở đâu?").type("FAQ")
                .tags(tags.apply(tagMap, new String[]{"payroll", "tax"})).audience("All employees")
            ...
            items.add(KnowledgeItem.builder().id("FAQ-003").title("Đi trễ bao nhiêu lần thì bị cảnh cáo?").type("FAQ")
                .tags(tags.apply(tagMap, new String[]{"behavior", "working_time"})).audience("All employees")
            ...
            items.add(KnowledgeItem.builder().id("FAQ-004").title("Đăng ký người phụ thuộc giảm trừ thuế TNCN").type("FAQ")
                .tags(tags.apply(tagMap, new String[]{"tax", "payroll", "benefit"})).audience("All employees")
            ...
            items.add(KnowledgeItem.builder().id("CHK-001").title("Onboarding Checklist – Ngày 1").type("Checklist")
                .tags(tags.apply(tagMap, new String[]{"onboarding"})).audience("New hires")
            ...
            items.add(KnowledgeItem.builder().id("CHK-004").title("IT Setup cho nhân viên mới").type("Checklist")
                .tags(tags.apply(tagMap, new String[]{"onboarding"})).audience("IT Support")
            ...
            items.add(KnowledgeItem.builder().id("CHK-005").title("Checklist dành cho Line Manager").type("Checklist")
                .tags(tags.apply(tagMap, new String[]{"onboarding"})).audience("Line Managers")
